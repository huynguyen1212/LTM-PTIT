...
//                f.setSourceName(rs.getString("name"));
                f.setSource(UserDAO.getUserById(rs.getInt("sourceId")));
    ...
    public Friend checkFriend(int currentId, int checkId) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                User userSource = UserDAO.getUserById(sourceIdL);
                User userTarget = UserDAO.getUserById(targetIdL);
                
                result.setSource(userSource);
                result.setTarget(userTarget);
