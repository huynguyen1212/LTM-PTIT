...
//                    }
                    ...
                    data.setGroup(group);
                    if(listFriends.get(row).getSource()!= null){
                        data.setUser(listFriends.get(row).getSource());
                    } else data.setUser(listFriends.get(row).getTarget());
                    
                    mySocket.sendData(new ObjectWrapper(ObjectWrapper.ADD_MEMBER, data));
                    //create new instance
...
//                    dispose();
        ...
        mySocket.getActiveFunction().add(new ObjectWrapper(ObjectWrapper.REPLY_LIST_FRIEND_TO_ADD_GROUP, this));
        ...
        new Timer().scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                ...
                for (int i = 0; i < listFriends.size(); i++) {
                    User infoFriend = new User();
                    if (listFriends.get(i).getSource() != null) {
                        infoFriend = listFriends.get(i).getSource();
                    } else {
                        infoFriend = listFriends.get(i).getTarget();
                    }
                    value[i][0] = infoFriend.getName();
                    value[i][1] = infoFriend.getUsername();
                    int indexToFindActive = -1;
                    int tempId = infoFriend.getId();
...
//        if (data.getPerformative() == ObjectWrapper.REPLY_LIST_FRIEND_TO_ADD_GROUP) {
...
//                    }
            ...
            if (data.getData().equals("true")) {
                ...
                JOptionPane.showMessageDialog(this, "User've already been in group!");
            }
        }
