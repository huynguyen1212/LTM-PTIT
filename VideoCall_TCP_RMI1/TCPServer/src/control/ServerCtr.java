...
//    -----RMI
    ...
    public void informDenyCall(PrivateCall privateCall) {
        ObjectWrapper data = new ObjectWrapper(ObjectWrapper.REPLY_CREATE_PRIVATE_CALL, "deny");
        for (ServerProcessing sp : myProcess) {
            if (sp.getIdUser() == privateCall.getSourceId()) {
                sp.sendData(data);
                System.out.println("tcp server inform deny call!");
                break;
    ...
    public void informAcceptCall(PrivateCall privateCall) {
        ObjectWrapper data = new ObjectWrapper(ObjectWrapper.REPLY_CREATE_PRIVATE_CALL, "accept");
        for (ServerProcessing sp : myProcess) {
            if (sp.getIdUser() == privateCall.getSourceId()) {
                sp.sendData(data);
                System.out.println("tcp server inform accept call!");
                break;
